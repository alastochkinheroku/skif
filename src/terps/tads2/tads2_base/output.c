...
struct out_stream_info
{
	...
	int html_start_sound;
	//Имя звукового файла
	char sound_file[MAXWIDTH];
	//Уровень проигрывания
	int sound_layer; //FOREGROUND
					//BGAMBIENT
					//AMBIENT 
					//BACKGROUND
	//Признак повторения композиции
	int sound_repeat;
	//Признак прерывания любого трека
	int sound_interrupt;
	//Признак прерывания любого трека на уровне
	int sound_cancel;
	//Уровень прерывания
	int sound_cancel_lvl;
	//Громкость музыки (относительная)
	int sound_volume;

    int html_start_soundgame; //включение режима звуковой игры
    int soundgame_timer_delay; //опрос по таймеру через мс
};
...
#define MAX_SND_CH 4
extern void skif_sound_play_chan(int layer, char* data, unsigned long sz, int repeat, int volume);
extern void skif_sound_stop_chan(int layer);
/* ------------------------------------------------------------------------ */
/*
                               ...
                               char *s, size_t slen)
{
    ...
    int		i, j;
    int		found_media;
    struct TadsSimpleResourse* simp_res;
        ...
        if (stream->html_mode_flag != HTML_MODE_NORMAL)
        {
            ...
            case HTML_MODE_TAG:
                ...
                case '>':
					...
					//Включение режима звуковой игры
					if (stream->html_start_soundgame == TRUE)
					{
						//sound_game_run = 1;
						
						stream->html_start_soundgame = FALSE;
					}

					//Включения звука, после прочтения набора атрибутов через bass.dll
					if (stream->html_start_sound == TRUE && (stream != &G_log_disp))
					{
						
						//включение музыки
						if ( (strlen(stream->sound_file)>0) && 
								(stream->sound_layer>=0) && (stream->sound_layer<MAX_SND_CH) &&
								!stream->sound_cancel && 
								!stream->sound_interrupt
								)
						{
							//поиск звука в карте
							simp_res = 0;
							for (j=0;j<resmap.total_res;j++)
							{
								if (strcmpi(stream->sound_file,resmap.res[j].filename)==0)
								{
									simp_res = &resmap.res[j];
									break;
								}
							}
                            //нашли звук в карте
							if (simp_res)
							{
                                skif_sound_play_chan(
                                    stream->sound_file,  //идентификатор звука
                                    stream->sound_layer, //номер слоя
                                    &resmap.file_memory[simp_res->offset], //данные
                                    simp_res->sz, //размер данных
                                    stream->sound_repeat, //признак зацикливания
                                    stream->sound_volume //звук относительный 0-100
                                );
							}
						}
						else if (stream->sound_interrupt) //Останавливаем все каналы
						{
							for (i=0;i<MAX_SND_CH;i++) {
                                skif_sound_stop_chan(i);
							}
						}
						else if ( (stream->sound_cancel)&& 
								    (stream->sound_cancel_lvl>=0) && (stream->sound_cancel_lvl<MAX_SND_CH) )
						{
                            skif_sound_stop_chan(stream->sound_cancel_lvl);
						}
					}
					stream->html_start_sound = FALSE;
                    break;
                ...
                default:
                    ...
                    if (outisal(c))
                    {
                        ...
                        *dst++ = '\0';
						...
						//Парсинг звука
						if (!stricmp(attrname, "src")
                             && stream->html_start_sound)
                        {
							strcpy(stream->sound_file,attrval);
						}
						else if (!stricmp(attrname, "layer")
                             && stream->html_start_sound)
                        {
							if (!stricmp(attrval, "foreground")) stream->sound_layer = 0;
							else if (!stricmp(attrval, "bgambient")) stream->sound_layer = 1;
							else if (!stricmp(attrval, "ambient")) stream->sound_layer = 2;
							else if (!stricmp(attrval, "background")) stream->sound_layer = 3;
						}
						else if (!stricmp(attrname, "repeat")
                             && stream->html_start_sound)
                        {
							stream->sound_repeat = 1;
						}
						else if (!stricmp(attrname, "volume")
                             && stream->html_start_sound)
                        {
							stream->sound_volume = atoi(attrval);
							if (stream->sound_volume<0) stream->sound_volume=0;
							else if (stream->sound_volume>100) stream->sound_volume=100;
						}
						else if (!stricmp(attrname, "cancel")
                             && stream->html_start_sound)
                        {
							stream->sound_cancel = 1;
							if (!stricmp(attrval, "foreground")) stream->sound_cancel_lvl= 0;
							else if (!stricmp(attrval, "bgambient")) stream->sound_cancel_lvl = 1;
							else if (!stricmp(attrval, "ambient")) stream->sound_cancel_lvl = 2;
							else if (!stricmp(attrval, "background")) stream->sound_cancel_lvl = 3;
						}
						else if (!stricmp(attrname, "interrupt")
                             && stream->html_start_sound)
                        {
							stream->sound_interrupt = 1;
						}
                 ...
                 && (c == '<' || c == '&'))
        {
            ...
            if (c == '<')
            {
                ...
                else if (!stricmp(tagbuf, "img") || !stricmp(tagbuf, "sound"))
                {
                    ...
                    //Запоминаем, что парсим звук
                    if (!stricmp(tagbuf, "sound")) {
                        stream->sound_layer = 0;
                        stream->sound_cancel = 0;
                        stream->sound_interrupt = 0;
                        stream->sound_repeat = 0;
                        stream->sound_volume = 100;
                        stream->sound_cancel_lvl = 0;
                        strcpy(stream->sound_file, "");
                        stream->html_start_sound = TRUE;
                    }
                    stream->html_allow_alt = TRUE;
                }
